            ...
            .onAppear(){
                ...
                if(firstAccess == false){
                    for i in 0...arrStudentModel.count - 1 {
                        viewModel.addStudent(nameSurnameIn: arrStudentModel[i])
                    }
                }
            }
        }
...
struct OnBoarding_Previews: PreviewProvider {
    static var previews: some View {
        OnBoarding(viewModel: CoreDataViewModel(), shouldShowOnboarding: .constant(false)).environment(\.managedObjectContext, PersistenceController.preview.container.viewContext)
    }
}
